...
test.describe('Inference Viewing', () => {
  ...
  test('should view inferences on entity detail page', async ({ page }) => {
    ...
    // Create source
    const sourceSlug = generateSourceName('inf-source').toLowerCase().replace(/\s+/g, '-');
    await page.goto('/sources/new');
    await page.getByLabel(/slug/i).fill(sourceSlug);
    await page.getByLabel(/summary.*en/i).fill('Source for inference test');
    ...
    await page.getByLabel(/summary.*en/i).fill('A person entity');
    await page.getByRole('button', { name: /create|submit/i }).click();
    ...
    await page.getByLabel(/summary.*en/i).fill('A company entity');
    await page.getByRole('button', { name: /create|submit/i }).click();
    await page.waitForURL(/\/entities\/[a-f0-9-]+/);
  ...
  test('should show inference scores', async ({ page }) => {
    ...
    const sourceSlug = generateSourceName('score-test').toLowerCase().replace(/\s+/g, '-');
    await page.goto('/sources/new');
    await page.getByLabel(/slug/i).fill(sourceSlug);
    await page.getByLabel(/summary.*en/i).fill('Source with high authority');
    await page.getByRole('button', { name: /create|submit/i }).click();
    await page.waitForURL(/\/sources\/[a-f0-9-]+/);
    ...
    await page.getByLabel(/summary.*en/i).fill('Entity with scored inferences');
    await page.getByRole('button', { name: /create|submit/i }).click();
    await page.waitForURL(/\/entities\/[a-f0-9-]+/);
  ...
  test('should view inference details', async ({ page }) => {
    ...
    await page.goto('/entities/new');
    await page.getByLabel(/slug/i).fill(entitySlug);
    await page.getByLabel(/summary.*en/i).fill('Entity for inference details');
    await page.getByRole('button', { name: /create|submit/i }).click();
    await page.waitForURL(/\/entities\/[a-f0-9-]+/);
