...
class TestSourceEndpoints:
    ...
    async def test_list_sources_pagination_default(self, override_get_db):
    ...
    async def test_list_sources_pagination_custom_limit(self, override_get_db):
        """Test pagination with custom limit."""
        app.dependency_overrides[get_db] = override_get_db
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                response = await client.get("/api/sources/?limit=10")
                assert response.status_code == status.HTTP_200_OK
                data = response.json()
                assert 'items' in data
                assert 'total' in data
    ...
    async def test_list_sources_pagination_with_offset(self, override_get_db):
        """Test pagination with offset."""
        app.dependency_overrides[get_db] = override_get_db
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                # Get first page
                response1 = await client.get("/api/sources/?limit=5&offset=0")
                assert response1.status_code == status.HTTP_200_OK
                page1 = response1.json()
                assert 'items' in page1

                # Get second page
                response2 = await client.get("/api/sources/?limit=5&offset=5")
    ...
    async def test_list_sources_pagination_limit_too_high(self, override_get_db):
        """Test that limit over 100 is rejected."""
        app.dependency_overrides[get_db] = override_get_db
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                response = await client.get("/api/sources/?limit=101")
                assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        finally:
            app.dependency_overrides.clear()

    async def test_list_sources_pagination_limit_zero(self, override_get_db):
        """Test that limit of 0 is rejected."""
        app.dependency_overrides[get_db] = override_get_db
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                response = await client.get("/api/sources/?limit=0")
                assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        finally:
            app.dependency_overrides.clear()

    async def test_list_sources_pagination_negative_offset(self, override_get_db):
        """Test that negative offset is rejected."""
        app.dependency_overrides[get_db] = override_get_db
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                response = await client.get("/api/sources/?offset=-1")
                assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        finally:
            app.dependency_overrides.clear()
